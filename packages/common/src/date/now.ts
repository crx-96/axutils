...
function currentParts(timezone?: string) {
...
export const Now = {
  ...
  plainDateISO(timezone?: string): Date {
  ...
  plainTimeISO(timezone?: string): Date {
    const { date } = currentParts(timezone);
  ...
  plainDateTimeISO(timezone?: string): Date {
    return currentParts(timezone).date;
  },

  /** 获取当前绝对时间点及其关联时区。 */
  zonedDateTimeISO(timezone?: string): ZonedDateTimeValue {
